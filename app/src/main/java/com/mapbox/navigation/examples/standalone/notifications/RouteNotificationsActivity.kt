...
class RouteNotificationsActivity : AppCompatActivity() {
    ...
    private fun showNotifications(route: NavigationRoute) {
        ...
        val notifications = route.upcomingRoadObjects.map { it.roadObject }.filterIsInstance<Notification>()
        notifications.forEach { notification ->
            // get notification start point
            val startPoint = ((notification.location as RouteAlertLocation).shape as LineString).coordinates().first()
            val text = notification.info.message ?: "Unknown notification"
            val notificationViewBindingStart = NotificationViewBinding.inflate(
                LayoutInflater.from(binding.mapView.context),
                binding.mapView,
                false
            )
